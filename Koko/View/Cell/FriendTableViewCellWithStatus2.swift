...
class FriendTableViewCellWithStatus2: UITableViewCell {
    ...
    lazy var inviteLabel: PaddingLabel = {
        let v = PaddingLabel()
        v.text = "邀請中"
        v.textColor = UIColor(rgb: 0x999999)
        v.font = UIFont(name: "PingFangTC-Medium", size: 14)
        v.layer.borderColor = UIColor(rgb: 0xc9c9c9).cgColor
        v.layer.borderWidth = 1.2
        v.layer.masksToBounds = true
        v.layer.cornerRadius = 2
        v.paddingLeft = 10
    ...
    private func subviewLayout() {
        ...
        transferButton.snp.makeConstraints {
            ...
            $0.right.equalTo(inviteLabel.snp.left).offset(-10)
        }
